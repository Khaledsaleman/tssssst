...
function getAdminIds(): number[] {
  ...
  const admins = [2003253093];
  if (envAdminId && !admins.includes(envAdminId)) {
    admins.push(envAdminId);
  }
  return admins;
}
...
async function isMemberOf(token: string, chatId: number, channelUsername: string): Promise<boolean> {
  try {
    ...
    const r = await fetch(
      `https://api.telegram.org/bot${token}/getChatMember?chat_id=${formattedUsername}&user_id=${chatId}`,
    );
...
router.post("/telegram/auth", async (req, res): Promise<void> => {
  ...
  res.status(200).json({ user: { ...user, balance }, isAdmin: adminIds.includes(user.id) });
});
...
router.get("/telegram/setup", async (_req, res) => {
  ...
  let webhookUrl = `${appUrl}/api/telegram/webhook`;
...
router.post(["/telegram/webhook", "/webhook"], async (req, res) => {
  ...
  if (update?.callback_query) {
    ...
    const isAdminCq = adminIds.includes(cqFrom.id);
  ...
  try {
    if (text === "/start" || text.startsWith("/start ")) {
      ...
      let webAppUrl = appUrl || "https://your-app.vercel.app";
      ...
      await sendMessage(token, chat_id, welcomeText, {
        reply_markup: {
          inline_keyboard: [
            [{ text: msgs.open_button, web_app: { url: webAppUrl } }],
          ],
        },
      });
